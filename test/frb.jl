...
# running across all 84 SMs. It should thus take 292 ms for 84
# frequencies. Current state for 84 frequencies is 113 ms, much better
# than expected. A bug is likely.
...
const map_fI_registers = Layout(Dict([BeamI(0) => Warp(0)
                                      BeamI(1) => Warp(1)
                                      BeamI(2) => Warp(2)
                                      BeamI(3) => Warp(3)
                                      BeamI(4) => Warp(4)
...
function fourier2!(steps::Vector{AbstractStep}, env::Environment)
    loop!(steps, env, :loopIdx2, :(Int32(0):Int32(2)), [BeamI(5), BeamI(6)]) do steps, env
        ...
        loop!(steps, env, :loopIdx1, :(Int32(0):Int32(2)), [BeamJ(5), BeamJ(6)]) do steps, env
            ...
            # (81)
            @assert env[:fJ] == Layout(Dict(Cplx(0) => Register(4), Polr(0) => Thread(1), BeamI(0) => Warp(0), BeamI(1) => Warp(1),
            ...
            @assert env[:fI4] == Layout(Dict(Polr(0) => Thread(1), BeamI(0) => Warp(0), BeamI(1) => Warp(1), BeamI(2) => Warp(2),
            ...
            # (86)
            @assert env[:fI8] == Layout(Dict(BeamI(0) => Warp(0), BeamI(1) => Warp(1), BeamI(2) => Warp(2), BeamI(3) => Warp(3),
...
function frb!(steps::Vector{AbstractStep}, env::Environment)
    ...
    loop!(steps, env, :loopIdx4, :(Int32(0):Int32(255)),
        ...
        loop!(steps, env, :loopIdx3, :(Int32(0):Int32(15)), [Time(3), Time(4), Time(5), Time(6)]) do steps, env
            ...
            sync_threads!(steps, env)
            fourier2!(steps, env)
            # TODO: update accumulators instead of storing
            return nothing
        end
        # TODO: Write result to global memory here
    end
    return nothing
end
...
function runcuda()
    ...
    E_mem = Int32[0x01010101 for i in 1:(2^30)]
    Gin_mem = Int32[0x02020202 for i in 1:(2^10)]
    Ans_mem = Int32[0x03030303 for i in 1:(2^9)]
    Aew_mem = Int32[0x04040404 for i in 1:(2^9)]
    ...
    F̃_shared = CuArray{Int32}(undef, 2^15)
